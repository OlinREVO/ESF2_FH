...
Person primarily responsible for this section:
    ...
    We will have isolation on our accelerator (throttle pedal potentiometers) by isolated CAN transceivers. Although we currently do not control the motor controller through CAN messages, we have a node next to the motor controller that listens to the throttle's data and outputs the same (isolated with an optoisolator) analog signal. The only use of the CAN system with the throttle information cleanliness of wiring and integration with the CAN communication system. The throttle information is not altered in any way. Please see figure \ref{mccschem} for details on the isolation between the motor controller controller (MCC) PCB Atmega16M1 and the motor controller.
